...
generateQuizBtn.addEventListener('click', async () => {
    ...
    let startPg = parseInt(startPageInput.value);
    let endPg = parseInt(endPageInput.value);

    // Validate inputs
    if (!startPg || !endPg || startPg > endPg || endPg > pdfDoc.numPages || startPg < 1) {
        alert(`Please enter a valid page range between 1 and ${pdfDoc.numPages}.`);
        return;
    }
    ...
    generateQuizBtn.innerHTML = "Generating... (takes a few secs)";
    ...
    try {
        ...
        const promptText = `
You are a medical physics professor creating a multiple choice quiz. 
Read the following slide text and generate exactly 5 multiple-choice questions.
Return ONLY a valid JSON array. Do not include any conversational text, and do not use markdown code blocks like \`\`\`json.
...
${extractedText}
        ...
        if (response.ok && data.choices && data.choices.length > 0) {
            ...
            // Clean up output using RegEx to grab only the array
...
function startQuiz() {
    ...
    // Takes up to 10 questions dynamically, allowing 5 from the AI generator
...
function showResults() {
    ...
    // Dynamic scoring threshold based on how many questions were played
