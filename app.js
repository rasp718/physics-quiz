...
generateQuizBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // Create a "Memory" of up to 40 recent questions so it doesn't duplicate them
        let existingQuestionsStr = "";
        if (quizData.length > 0) {
            const recentQs = quizData.slice(-40).map(q => q.question);
...
6. Return ONLY a valid JSON array. Do not include any conversational text, and do not use markdown code blocks like \`\`\`json.
${existingQuestionsStr}
...
${extractedText}
        ...
        const response = await fetch(myProxyUrl, {
            ...
            body: JSON.stringify({
                ...
                max_tokens: 3000,
                temperature: 0.7 // Increased from 0.2 to encourage variety
            })
        });
        ...
        if (response.ok && data.choices && data.choices.length > 0) {
            ...
            // Append new questions to the bank instead of replacing it!
            quizData = quizData.concat(newQuestions);
            
            document.getElementById('subject-title').innerText = "AI QUIZ READY";
            document.getElementById('subject-subtitle').innerText = `Quiz bank now has ${quizData.length} questions!`;
