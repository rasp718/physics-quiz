...
// Load the PDF Document behind the scenes immediately
pdfjsLib.getDocument('slides.pdf').promise.then(function(pdfDoc_) {
    pdfDoc = pdfDoc_;
    console.log("slides.pdf loaded successfully! Total pages: " + pdfDoc.numPages);
}).catch(function(error) {
    console.log("No slides.pdf found. Slide viewing will be disabled.");
});
...
// Modal Logic
closeModalBtn.addEventListener('click', () => slideModal.classList.add('hidden'));
slideModal.addEventListener('click', (e) => {
    if (e.target === slideModal) slideModal.classList.add('hidden');
});
    ...
    if (q.page && pdfDoc) {
        ...
        viewSlideBtn.onclick = () => {
            ...
            pdfDoc.getPage(q.page).then(function(page) {
                ...
                // Scale 1.5 gives high-resolution rendering on mobile
