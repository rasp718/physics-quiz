...
fetch('questions.json').then(res => res.json()).then(data => { 
    ...
    showExportButton(); // Show export button if initial fetch succeeds
}).catch(() => {});
...
${extractedText}
        ...
        if (response.ok && data.choices && data.choices.length > 0) {
            ...
            showExportButton(); // Show export button if it wasn't there
            startBtn.classList.remove('hidden'); 
        } else {
...
async function reviewQuestionAI(q, downBtn) {
    ...
    2. PEDAGOGICAL QUALITY: Is it "slop"? 
       ...
       - SLOP CHECK: Are the options confusing, overlapping, or missing proper math formatting (e.g., missing <sup>2</sup> for exponents)?
    ...
    DECISION CRITERIA:
    - If the question lacks proper HTML formatting for math (superscripts/subscripts): Action = "FIX".
    ...
    try {
        ...
        if (response.ok && data.choices) {
            ...
            if (result.action === "DELETE") {
                ...
                const fixDetails = `Proposed Correction:\n\nQ: ${fq.question}\n- ${fq.options[0]}\n- ${fq.options[1]}\n- ${fq.options[2]}\n- ${fq.options[3]}\n\nCorrect Answer: ${fq.options[fq.answer]}`;
