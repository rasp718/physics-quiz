...
1. NO SPOILERS: Never include hints about the answer format in the question text. (e.g., NEVER start a question with "Both A and B:", "True or False:", or "Which two of the following...").
2. QUESTION PURITY: The question should be a clean, direct inquiry. 
   - BAD: "Both A and B: What are the components of X?" 
   - GOOD: "What are the two primary components of an X-ray tube emission spectrum?"
3. COMBINATION OPTIONS: For exactly 2 questions, one of the 4 options (usually option D) should be "Both A and B" or "All of the above." 
   - Ensure the individual options (A and B) are plausible but incomplete on their own in the context of the question.
4. DISTRACTOR QUALITY: Distractors must be scientific terms found in the slides but incorrect for this specific question. Avoid "None of the above" unless specified.
...
// --- FIX: SOFT RESET INSTEAD OF RELOAD ---
document.getElementById('restart-btn').onclick = () => {
    // Hide the result screen and bring back the setup screen
    resultContainer.classList.add('hidden');
    setupContainer.classList.remove('hidden');
    
    // Fix the visual state of the start button
    startBtn.classList.remove('is-squished');
    
    // Update the text to confirm your massive bank is still there
    document.getElementById('subject-subtitle').innerText = `${quizData.length} questions in bank. Ready to begin.`;
};
...
function startQuiz() {
    ...
    // Prioritize just generated questions, fill the rest randomly
...
function checkAnswer(selected, correct, btn, q) {
    ...
    // Inject Feedback (Thumbs) UI dynamically
    ...
    feedbackContainer.innerHTML = ''; // Reset for this question
...
async function reviewQuestionAI(q, downBtn) {
    ...
    1. FACTUAL ACCURACY: Does it match the slide text?
    2. PEDAGOGICAL QUALITY: Is it "slop"? 
       ...
       - SLOP CHECK: Is the answer obvious because of how the question is phrased?
    ...
    DECISION CRITERIA:
    - If the question text contains "Both A and B" or similar formatting hints: Action = "FIX".
    - If the facts are wrong: Action = "FIX".
    - If it's a "gimme" question with no educational value: Action = "DELETE".
    - If it is clear, challenging, and spoiler-free: Action = "KEEP".
    ...
    try {
        ...
        if (response.ok && data.choices) {
            ...
            if (result.action === "DELETE") {
                quizData = quizData.filter(item => item.question !== q.question); // Remove from bank
                ...
                if (idx !== -1) {
                    quizData[idx] = result.fixed_question; // Fix in bank
                }
                downBtn.innerText = "🔧 Fixed";
                alert("AI Review: " + result.reason + "\n\nResult: Question has been corrected for future runs.");
            } else {
                downBtn.innerText = "✅ Kept";
                alert("AI Review: " + result.reason + "\n\nResult: Question verified as accurate. Kept in bank.");
            }
        }
    } catch (err) {
