...
5. Return ONLY a valid JSON array. Do not include any conversational text, and do not use markdown code blocks like \`\`\`json.
...
function showQuestion() {
    ...
    // Changed to innerHTML to support sub/sup tags
    ...
    q.options.forEach((option, index) => {
        ...
        // Changed to innerHTML to support sub/sup tags in options
...
function checkAnswer(selected, correct, btn, q) {
    ...
    // Changed to innerHTML to support sub/sup tags in explanations
...
async function fetchAIExplanation(q) {
    ...
    try {
        ...
        if (response.ok && data.choices && data.choices.length > 0) {
            ...
            // Automatically clean up markdown bold and common sub/sup structures to HTML
