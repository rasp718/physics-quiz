...
// --- FULLSCREEN & SLIDE INTERACTION ---
function enterFullScreen() {
    const elem = document.documentElement;
    if (elem.requestFullscreen) { elem.requestFullscreen().catch(() => {}); } 
    else if (elem.webkitRequestFullscreen) { elem.webkitRequestFullscreen(); }
    
    // Fallback attempt to hide iOS Safari bar
...
closeModalBtn.addEventListener('click', () => {
    ...
    pdfCanvas.classList.remove('zoomed'); // reset zoom state
    exitFullScreen();
});

// Click on the slide canvas specifically to native-fullscreen and zoom
    ...
    if (q.page && pdfDoc) {
        viewSlideBtn.onclick = () => {
            ...
            pdfDoc.getPage(q.page).then(function(page) {
                ...
                // Extremely crisp rendering for zooming
