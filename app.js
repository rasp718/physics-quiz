...
window.addEventListener('load', () => {
    // --- SAFARI iOS 18+ HAPTIC INJECTION WORKAROUND ---
    if (isIOS) {
        try {
            ...
            console.warn("Failed to inject iOS haptic workaround", e);
        }
    }
...
// --- HAPTICS ENGINE (With iOS 18+ Safari Workaround) ---
function triggerHaptic(type) {
    try {
        if (isIOS && iosHapticLabel) {
            // iOS 18+ Safari Switch Workaround
            if (type === 'correct') {
                iosHapticLabel.click();
                setTimeout(() => { if (iosHapticLabel) iosHapticLabel.click(); }, 120);
            } else if (type === 'wrong') {
                iosHapticLabel.click();
                setTimeout(() => { if (iosHapticLabel) iosHapticLabel.click(); }, 100);
                setTimeout(() => { if (iosHapticLabel) iosHapticLabel.click(); }, 200);
            } else {
                iosHapticLabel.click();
            }
        ...
        console.warn("Haptics restricted by browser.");
    }
}
...
closeModalBtn.addEventListener('click', () => {
    ...
    pdfCanvas.classList.remove('zoomed'); 
    exitFullScreen();
});
...
pdfCanvas.addEventListener('click', (e) => {
    ...
    triggerHaptic('tap');
    enterFullScreen(); 
    pdfCanvas.classList.toggle('zoomed');
});
...
${extractedText}
        ...
        if (response.ok && data.choices && data.choices.length > 0) {
            ...
            triggerHaptic('correct'); // Vibrate heavily on success!
        ...
        triggerHaptic('wrong'); // Vibrate heavily on error
...
function showQuestion() {
    ...
    if (q.page && pdfDoc) {
        viewSlideBtn.onclick = () => {
            ...
            // Auto-zoom if in landscape mode natively
            if (window.matchMedia("(orientation: landscape)").matches) {
                pdfCanvas.classList.add('zoomed');
            } else {
                pdfCanvas.classList.remove('zoomed');
            }
            ...
            pdfDoc.getPage(q.page).then(function(page) {
                ...
                const scale = Math.max(2.5, window.devicePixelRatio || 2.5);
...
function showResults() {
    triggerHaptic('correct'); // Celebrate!
...
window.matchMedia("(orientation: landscape)").addEventListener("change", function(e) {
    ...
    if (e.matches) {
        if (slideModal.classList.contains('hidden') && viewSlideBtn.onclick) {
            viewSlideBtn.onclick(); 
        }
    } else {
