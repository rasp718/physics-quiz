...
const quizAnimWrapper = document.getElementById('quiz-anim-wrapper'); // Controls Swipes
...
startBtn.onclick = () => {
    // 3D squish before running logic
...
function triggerHaptic(type) {
    ...
    if (type === 'correct') navigator.vibrate([30, 50, 30]); // Happy bouncy rumble
    else navigator.vibrate([100, 50, 100]); // Angry double tap
}
...
function showQuestion() {
    ...
    // The "Card Deal" creation
    q.options.forEach((option, index) => {
        const btn = document.createElement('button');
        btn.innerText = option;
        btn.className = 'option-btn 3d-btn deal-anim';
        btn.style.setProperty('--i', index); // Sets the staggered delay
        
        btn.onclick = () => {
            btn.classList.add('is-squished'); // Physical squish
            setTimeout(() => {
...
function checkAnswer(selected, correct, btn, q) {
    ...
    buttons.forEach(b => {
        b.disabled = true;
        b.classList.remove('deal-anim'); // Clean up animation classes
    });
    ...
    if (selected === correct) {
        ...
        // Breathing fire logic
        if(streak >= 3) fireIcon.classList.add('breathing-fire');
        
    } else {
        btn.classList.add('wrong', 'shake');
        buttons[correct].classList.add('correct'); // Show right answer
        streak = 0;
...
function loadNextQuestion() {
    ...
    }, 250); // Wait for swipe to finish before changing data
}
