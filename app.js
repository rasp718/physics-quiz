...
function checkAnswer(selected, correct, btn, q) {
    ...
    // JUST REVEAL THE BUTTON (Logic is now prepped in showQuestion)
    if (q.page && pdfDoc) {
        viewSlideBtn.classList.remove('hidden');
    }
...
window.matchMedia("(orientation: landscape)").addEventListener("change", function(e) {
    if (quizContainer.classList.contains('hidden')) return; // Ignore if not in the quiz
    
    const q = currentQuiz[currentQuestionIndex];
    if (!q || !q.page || !pdfDoc) return; // Ignore if no slide exists
    
    if (e.matches) {
        // Device turned sideways -> Automatically show the full-screen slide
        if (slideModal.classList.contains('hidden') && viewSlideBtn.onclick) {
            viewSlideBtn.onclick(); 
        }
    } else {
        // Device turned upright -> Close the slide back to the quiz
