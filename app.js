...
async function reviewQuestionAI(q, downBtn) {
    ...
    const promptText = `
    A user flagged this multiple-choice question as potentially flawed, confusing, or a "trick" question.
    
    Question: ${q.question}
    Options: ${JSON.stringify(q.options)}
    Correct Answer Index: ${q.answer}
    Explanation: ${q.explanation}
    Slide Content (Page ${q.page}):
    "${slideText}"

    Evaluate the question based ONLY on the slide content provided.
    1. If the question is factually incorrect, overly confusing, or has a frustrating "Both A and B" trick where the individual answers look too perfect, FIX it to be unambiguous.
    2. If it is hopelessly flawed and cannot be logically saved, mark it for DELETION.
    3. If it is actually perfectly fine, accurate, and fair, mark it to KEEP.
