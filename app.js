...
generateQuizBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // Pass ALL existing questions so the AI knows what to avoid
        let existingQuestionsStr = "";
        if (quizData.length > 0) {
            const allQs = quizData.map(q => q.question);
            existingQuestionsStr = `\nIMPORTANT AVOIDANCE RULE:\nDo NOT generate questions that are highly similar to these existing questions:\n${allQs.map(q => "- " + q).join('\n')}\n`;
        }

        const promptText = `
You are an expert medical physics exam writer creating a multiple choice quiz. 
Read the following slide text and generate exactly 10 NEW multiple-choice questions.

RULES FOR QUESTIONS:
1. Make the correct answer unambiguously clear.
2. COMBINATION OPTIONS: For at least 2 or 3 of the questions, design them so that more than one statement is true. In these cases, make the correct answer a combination choice (e.g., "Both A and B" or "Both A and C"). Ensure the specific option letters align logically with the array placement.
3. The incorrect options (distractors) must be plausible but definitively wrong.
4. Format superscripts and subscripts using HTML tags (e.g., B<sub>0</sub>, x<sup>2</sup>, &mu;, &lambda;).
5. Dig deep into the slides. Choose a wide variety of topics, concepts, and pages.
6. Each question MUST include an "explanation" field (string) explaining the correct answer, and a "page" field (integer) indicating the slide number it was sourced from.
7. SELF-REVIEW STEP: First, write a brief "SELF-REVIEW:" section. Analyze the 10 questions you intend to ask. Check that they make logical sense, the "Both" options refer to the correct choices, and they are highly accurate based ONLY on the provided slides. 
8. JSON FORMAT: After your self-review, provide the questions. You MUST wrap your final JSON array inside exact <FINAL_JSON> and </FINAL_JSON> tags. The JSON must be an array of objects with exact keys: "question", "options" (array of exactly 4 strings), "answer" (integer 0-3), "explanation", and "page".
${existingQuestionsStr}
...
${extractedText}
        ...
        const response = await fetch(myProxyUrl, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    { role: "system", content: "You strictly follow instructions. You write out a self-review first, then output the final JSON array wrapped in <FINAL_JSON> tags." },
        ...
        if (response.ok && data.choices && data.choices.length > 0) {
            ...
            // Extract the JSON specifically from the custom tags to avoid grabbing lists from the review text
            const jsonMatch = aiOutput.match(/<FINAL_JSON>([\s\S]*?)<\/FINAL_JSON>/); 
            let finalJsonString = "";
            if (jsonMatch) {
                finalJsonString = jsonMatch[1].trim();
            } else {
                // Fallback if the AI forgot the tags but hopefully used markdown
            ...
            // Deduplication logic: Strip punctuation and spaces to find duplicates
            ...
            newQuestions.forEach(q => {
                ...
                // Only add to the bank if the exact/highly-similar string doesn't exist
            ...
            document.getElementById('subject-subtitle').innerText = `Added ${addedCount} new unique questions! Bank now has ${quizData.length} total.`;
