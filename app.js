...
fileInput.addEventListener('change', function(e) {
    ...
    // Updates the subtitle text instead of the "Scientia" title
    document.getElementById('subject-title').innerText = file.name.replace('.json', '').toUpperCase();
    document.getElementById('subject-subtitle').innerText = "Ready to start.";

    const reader = new FileReader();
    reader.onload = function(e) {
        try {
            quizData = JSON.parse(e.target.result);
            // Quick confirmation instead of an intrusive popup
            document.getElementById('start-btn').innerText = `Start (${quizData.length} Qs)`;
        } catch (err) {
...
function showResults() {
    ...
    const confettiColors = isLightMode 
        ? ['#292524', '#78716c', '#e7e5e4'] 
        : ['#ffffff', '#a3a3a3', '#555555']; 
