...
async function fetchAIExplanation(q) {
    ...
    const promptText = `Question: ${q.question}\nCorrect Answer: ${q.options[q.answer]}\nBrief Explanation: ${q.explanation}\n\nPlease act as a friendly physics professor. Explain the underlying medical physics concept deeper, but keep it very easy to understand in exactly 3 to 4 short sentences.`;

    try {
        // Route through a free CORS proxy to bypass NVIDIA's browser restrictions
        ...
        const response = await fetch(proxyUrl + encodeURIComponent(targetUrl), {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    { role: "system", content: "You are a helpful medical physics tutor." },
        ...
        // Check if the response is successful and has AI text
        if (response.ok && data.choices && data.choices.length > 0) {
            aiExplanationBox.innerText = "✨ " + data.choices[0].message.content.trim();
        } else {
            console.error("NVIDIA API Error:", data);
            aiExplanationBox.innerText = "Error: Invalid API key or out of credits. Please check your settings.";
        }
    } catch (err) {
        console.error("Fetch Error:", err);
        aiExplanationBox.innerText = "Connection error. The CORS proxy may be experiencing high traffic, or your network is blocking the request.";
    }
