...
document.addEventListener('keydown', (e) => {
    // Ignore keystrokes if the user is typing in an input field (like the API key box)
    ...
    if (e.key === 'ArrowRight') {
        if (!quizContainer.classList.contains('hidden') && !navActions.classList.contains('hidden')) {
            // Only trigger next if the question has been answered (next-btn is visible)
            if (!nextBtn.classList.contains('hidden')) loadNextQuestion();
        } else if (!resultContainer.classList.contains('hidden')) {
            // If on the result screen, right arrow returns to main menu
...
function showExportButton() {
    let exportBtn = document.getElementById('export-bank-btn');
    if (!exportBtn) {
        exportBtn = document.createElement('button');
        exportBtn.id = 'export-bank-btn';
        exportBtn.className = 'upload-btn 3d-btn';
        exportBtn.style.marginTop = '10px';
        exportBtn.innerHTML = '<span class="icon">💾</span> Save Bank to File';
        exportBtn.onclick = () => {
        ...
        document.querySelector('.ai-gen-section').appendChild(exportBtn);
    }
}
...
function showQuestion() {
    ...
    q.options.forEach((option, index) => {
        ...
        btn.onclick = () => {
            // Prevent double-answering if already answered
    ...
    // If the user already answered this question (navigating backwards/forwards)
    if (userAnswers[currentQuestionIndex] !== undefined) {
        renderAnswerState(userAnswers[currentQuestionIndex], q, false);
    } else {
        // Unanswered state - show Back button if applicable
        if (currentQuestionIndex > 0) {
            navActions.classList.remove('hidden');
            prevBtn.classList.remove('hidden');
            nextBtn.classList.add('hidden'); // Cannot go forward until answered
        }
    }
}

function checkAnswer(selected, correct, btn, q) {
    userAnswers[currentQuestionIndex] = selected; // Save the answer in history
    renderAnswerState(selected, q, true); // true = trigger animations and haptics
}
...
function renderAnswerState(selected, q, animate = false) {
    ...
    // Calculate streak backwards from the current question index
    let currentStreak = 0;
    ...
    // Inject AI Review Feedback (Thumbs) UI dynamically
    ...
    navActions.classList.remove('hidden');
    nextBtn.classList.remove('hidden'); // Now that it's answered, they can continue
...
function showResults() {
    ...
    // Calculate final score dynamically based on userAnswers history
    let finalScore = 0;
    let incorrectQs = [];
    ...
    if (incorrectQs.length > 0) {
        ...
        retryIncorrectBtn.onclick = () => {
            ...
            setTimeout(() => {
                ...
                currentQuiz = incorrectQs; // Launch new sub-quiz with only the missed questions
                resultContainer.classList.add('hidden');
                startQuiz('retry'); 
            }, 150);
        };
